...
-ساختار متدهای اینترفیس HandlingStrategy: برای پوشش کامل چرخه حیات یک تیکت، این اینترفیس دارای دو متد کلیدی است که در مراحل مختلف فراخوانی می‌شوند:
متد assign() (مرحله ارجاع):
کاربرد: این متد در وضعیت AssignedState فراخوانی می‌شود.
...
متد respond() (مرحله پاسخ‌دهی):
کاربرد: این متد در وضعیت InProgressState فراخوانی می‌شود.
...
• روش اعمال:
- یک کلاس TicketFactory ،abstract می‌سازیم که متد استاتیک createTicket را دارد .
- این متد پارامترهاي ورودي (Id، کانال و نوع) را می‌گیرد و بر اساس آنها، آبجکت Ticketرا می‌سازد و برمی‌گرداند.
